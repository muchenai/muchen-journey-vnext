...
for path in "$ca" "$facts_script" "$target_env"; do
...
assert value["migration"] == "0019_wp30_invitation_control"
assert value["counts"]
...
WP15_BACKUP_KEY="$WP15_BACKUP_KEY" python3 - "$manifest" "$WP31_RUN_ID" \
  "$plain_sha" "$encrypted_sha" "$facts_sha" <<'PY'
import hashlib, hmac, json, os, sys
path, run_id, plain_sha, encrypted_sha, facts_sha = sys.argv[1:]
body = {
    "schema_version": 1,
    "run_id": run_id,
...
PY
chmod 0600 "$encrypted" "$facts" "$manifest"
