...
def test_reviewed_logout_feedback_runtime_surface_is_allowed():
    validate_changes(list(ALLOWED_RUNTIME_CHANGES))


def test_reviewed_logout_feedback_files_are_explicitly_allowed():
    validate_changes([
...
@pytest.mark.parametrize("path", ["migrations/versions/new.py", "apps/api/journey_api/models.py",
    ...
    "apps/web/src/app/app/result/page.tsx", "apps/web/src/lib/server/session.ts"])
