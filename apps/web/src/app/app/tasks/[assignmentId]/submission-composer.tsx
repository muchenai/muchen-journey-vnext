...
export function SubmissionComposer({
  ...
  return (
    <form ref={formRef} action={submitAction} className="submission-composer">
      ...
      {submitState.success ? <div className="success-text" role="status" aria-live="polite">{submitState.success} <a href="/app">返回旅程地图</a></div> : null}
      {submitState.success ? <div className="success-text" role="status" aria-live="polite">{submitState.success} <a href="/app">返回旅程地图</a></div> : null}

      <p id="submission-save-status" className="draft-save-status" role="status" aria-live="polite">
        {!isOnline ? "当前离线：修改已保留为本浏览器未同步副本；恢复网络后再保存或正式提交。" : draftPending ? "正在保存草稿到服务器…" : draftState.savedAt ? `草稿已保存到服务器 · revision ${draftState.draftRevision} · ${new Date(draftState.savedAt).toLocaleString("zh-CN")}` : initialDraftUpdatedAt ? `服务器草稿 revision ${initialDraftRevision} · ${new Date(initialDraftUpdatedAt).toLocaleString("zh-CN")}` : "尚未保存；编辑后会自动保存到服务器，并保留本地恢复副本。"}
      </p>
