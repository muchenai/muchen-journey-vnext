#!/usr/bin/env python3
"""Create the exact owner-only WP-15 wartime production bundle."""
...
PRESERVED_FAILED_DATABASE = "journey_next_production"
MIGRATION = "0019_wp30_invitation_control"
IMAGES = {
    "API_IMAGE": "ghcr.io/muchenai2024-creator/muchen-journey-vnext-api@sha256:2a053bad89bea8c06daba6e929af49a4804cc06a2321e49e93858f1f4fda6a6c",
    "WEB_IMAGE": "ghcr.io/muchenai2024-creator/muchen-journey-vnext-web@sha256:a3335542f74d09f4bc394119cee81ba7b866edc6ef041f3f4444949d271e2aee",
    "WORKER_IMAGE": "ghcr.io/muchenai2024-creator/muchen-journey-vnext-worker@sha256:2ef3cd1b05c545810929a3136ac8259042f6b6c586ccb8c59af90c579bfd9f38",
}
...
class PrepareWartimeError(RuntimeError):
    pass
...
def require(name: str, minimum: int = 1) -> str:
    ...
    if len(value) < minimum or "\n" in value or "\r" in value:
        raise PrepareWartimeError(f"required environment variable is invalid: {name}")
    return value
...
def write_env(path: Path, values: dict[str, str], mode: int = 0o600) -> None:
    for key, value in values.items():
        if not re.fullmatch(r"[A-Z][A-Z0-9_]*", key) or "\n" in value or "\r" in value:
            raise PrepareWartimeError(f"unsafe env value: {key}")
...
def prepare(output: Path, host: str, port: int) -> None:
    if not re.fullmatch(r"[A-Za-z0-9.-]+", host) or host in {"localhost", "127.0.0.1"}:
        raise PrepareWartimeError("RDS host is invalid")
    if not 1 <= port <= 65535:
        raise PrepareWartimeError("RDS port is invalid")
    ...
    if len({session_secret, invite_secret, import_key, backup_key}) != 4:
        raise PrepareWartimeError("production application and backup secrets must be independent")

    output.mkdir(parents=True, exist_ok=False, mode=0o700)
    secrets = output / "secrets"
    secrets.mkdir(mode=0o700)
    runtime_url = dsn(
        "journey_next_runtime", runtime_password, host, port, PRODUCTION_DATABASE
    )
    migration_url = dsn(
        "journey_next_migrator", migration_password, host, port, PRODUCTION_DATABASE
    )
    source_url = dsn(
        "journey_next_migrator", migration_password, host, port, STAGING_DATABASE
    )
    shared = {
        "APP_ENV": "production",
    ...
    write_env(
        secrets / "worker.env",
        {
            ...
            "NOTIFICATION_RECIPIENT_KEY": recipient_key,
            "FEISHU_NOTIFICATION_APP_ID": notify_app_id,
            "FEISHU_NOTIFICATION_APP_SECRET": notify_app_secret,
    ...
    write_env(
        secrets / "web.env",
        {
            ...
            "APP_RELEASE": CANDIDATE,
            "CONFIG_SCHEMA_VERSION": "3",
            "API_INTERNAL_URL": "http://production-api:8000",
            "ALLOW_FIXTURE_IDENTITY": "false",
        },
    )
    write_env(
    ...
    try:
        ...
        raise PrepareWartimeError("RDS CA is invalid Base64") from error
    if b"-----BEGIN CERTIFICATE-----" not in ca:
        raise PrepareWartimeError("RDS CA is not PEM")
    ...
    print("WP15_WARTIME_BUNDLE=READY secret_values_printed=false")
...
def main() -> None:
    ...
    except (OSError, PrepareWartimeError) as error:
        raise SystemExit(f"WP15_WARTIME_PREPARE_ERROR: {error}") from error
