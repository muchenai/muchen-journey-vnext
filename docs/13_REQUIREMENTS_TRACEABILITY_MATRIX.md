...
| WP-08 物理独立 Staging | ISO-MUST-003/004/007/008/009/012；DEC-003/013/014；AT-ISO-002/003/005；AT-ARCH-002/003/005 | `make wp08-staging-readiness`、`make wp08-staging-apply-check`、`make wp08-workflow-check`、Terraform validate、saved-plan destroy/replacement guard、独立 Web `/health/ready` runtime check、匿名 `/ops = 401`、首次失败容器清理、候选源码 Web 合同门禁，以及 Worker database-only 配置/完整 API secret 双向负测；DoR 与路径证据见 25/26 | 唯一 d407… deploy run 30138363837 因 Worker 错误加载 API secret 配置失败，容器清理与 SSH 撤销通过且未重试。PR #41 已合入最小权限修复；Mainline Candidate Gate 30139385352 为 dad44cc… 完成完整 CI、SBOM、GHCR push 与三摘要验证，本绑定变更原子更新机器合同且不部署 | `ALPHA_PILOT_CANDIDATE_BOUND / DEPLOY_NOT_AUTHORIZED`；d407… 禁止重试，新候选须在绑定 PR 合入后另行精确授权；WP-09、真人 Alpha UAT 与整体发布仍 `NO_GO` |
