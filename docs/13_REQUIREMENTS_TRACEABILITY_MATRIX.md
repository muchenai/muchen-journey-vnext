...
| WP-12 候选硬化与灾备 | DEC-008/013/014/019；REQ-NFR-005/008/010；AT-DATA-008；AT-ISO-007；AT-ARCH-001/007 | 非本地 fail closed；最小权限与 source map/依赖/secret audit；threat model；DEC-008 policy、migration `0014`、PII-free plan；Operator 数据权利登记/查询/legal hold/受限拒绝与脱敏审计；loopback-only benchmark；WP-12B 20 组织/500 Learner/50 并发真实 HTTP 负载（应用性能在北京现有 ECS Compose 网络内测量，公开 HTTPS 仅核对可达性/release）与数据库隔离审计；加密本地备份、空白恢复、事实指纹和 `0013↔0014` 零新增事实回滚拒绝；`tests/test_wp12_*`；见 30/32 | 硬化、threat model、生命周期 PLAN、Operator 台账、本地性能和本地隔离恢复已验证。staging run `30482295111` 完成 20 组织/500 Learner、500/500 Reviewer 队列和 10,562 请求，5xx/409/cross-org leak/unexpected response 均为 0，但受美国西部 runner→北京跨境基线影响，7 个端点 p95 超过 1 秒；旧顺序跳过数据库 audit，身份退役、证据上传和 SSH 关闭 PASS。测量位置与 audit-always 修复待合入，容量门禁未关闭。物理 APPLY/真实删除、真实 RPO/RTO、真实故障卡、异机恢复仍未执行；独立故障域按 DEC-019 延期 | `IN_PROGRESS / WP12B_STAGING_PERFORMANCE_FAIL_MEASUREMENT_FIX_PENDING / DATA_RIGHTS_LEDGER_READY_APPLY_DISABLED / OFF_HOST_RESTORE_NOT_RUN / PRODUCTION_NO_GO`；不得记 `RC_TECHNICALLY_READY` |
