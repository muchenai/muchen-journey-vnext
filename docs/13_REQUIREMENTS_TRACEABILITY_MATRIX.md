...
| WP-08 物理独立 Staging | ISO-MUST-003/004/007/008/009/012；DEC-003/013/014；AT-ISO-002/003/005；AT-ARCH-002/003/005 | `make wp08-staging-readiness`、`make wp08-staging-apply-check`、`make wp08-workflow-check`、Terraform validate、saved-plan destroy/replacement guard、人工网络边界核验，以及 migration 前的非 root 容器 CA 读取门禁；DoR 与路径证据见 25/26 | provision 已冻结；人工只读核验已确认 ECS/AllowList/安全组/RDS/VPC 一致、RDS 无公网且强制 TLS。唯一 deploy run 30109954801 拉取四镜像成功，但 UID 10001 容器无法读取 root-only CA，数据库连接和 migration 均未开始；SSH 已关闭且未重试。CA 改为公开证书适用的 `0444`，其他 secret 保持 `0600`，完整 API/迁移 126 项及真实非 root 容器读取复验通过 | `ALPHA_PILOT_CA_READABILITY_FIX_READY`；修复合入后需新的单次 deploy 授权，WP-09 未激活，整体发布仍 `NO_GO` |
