...
| WP-12 候选硬化与灾备 | DEC-008/013/014/019；REQ-NFR-005/008/010；AT-DATA-008；AT-ISO-007；AT-ARCH-001/007 | 非本地 fail closed；最小权限与 source map/依赖/secret audit；threat model；DEC-008 policy、migration `0014`、PII-free plan；Operator 数据权利登记/查询/legal hold/受限拒绝与脱敏审计；loopback-only benchmark；WP-12B 20 组织/500 Learner/50 并发真实 HTTP 负载（应用性能在北京现有 ECS API 容器内测量，公开 HTTPS 仅核对可达性/release）与数据库隔离审计；加密本地备份、空白恢复、事实指纹和 `0013↔0014` 零新增事实回滚拒绝；`tests/test_wp12_*`；见 30/32 | 硬化、threat model、生命周期 PLAN、Operator 台账、本地性能和本地隔离恢复已验证。staging run `30487668744` 首次完成北京容器内完整性能测量：20 组织/500 Learner、50 峰值并发、10,561 请求，HTTP 5xx/409、跨组织泄漏和意外响应均为 0；数据库 audit、身份退役、证据上传和 SSH 关闭均 PASS。真实性能门禁因 `reviewer.review_start` p95 1.365 秒、`reviewer.review_finalize` p95 1.502 秒超过 1 秒预算而 FAIL，其余端点均 PASS；根因收敛为 Reviewer 状态迁移的多次数据库往返。最小写路径优化已由 PR #86 合入，新候选 `674e51d8ed67f9c29c3d04693376c9ba6f1114e5` 经 Mainline run `30489417625` 完成三镜像和摘要验证，并由唯一 deploy run `30506961105` 在冻结 staging 成功部署；精确 release、匿名受保护路由、三档视口 browser smoke 与 SSH 关闭均 PASS。该候选尚未获准执行 WP-12B 负载。物理 APPLY/真实删除、真实 RPO/RTO、真实故障卡、异机恢复仍未执行；独立故障域按 DEC-019 延期 | `IN_PROGRESS / WP12B_REVIEWER_WRITE_PATH_CANDIDATE_DEPLOYED_LOAD_NOT_AUTHORIZED / DATA_RIGHTS_LEDGER_READY_APPLY_DISABLED / OFF_HOST_RESTORE_NOT_RUN / PRODUCTION_NO_GO`；不得记 `RC_TECHNICALLY_READY` |
