...
| REQ-BR-011 版本化探索营旅程 | JRN-007；`/app` 旅程投影 | JourneyDefinition/Version/StageVersion、Enrollment、Resolver | Journey publish/read；Current Action | organization + audience；发布后不可变；在途版本固定 | AT-PRODUCT-001；AT-DATA-009；AT-UX-010 | WP-19 已实现 schema/API/migration、Alpha 回填、有序 Assignment 与只读进度；正式内容和完整旅程仍未激活；见 35 |
...
| WP-19 Journey Composition | DEC-024；REQ-BR-011；REQ-NFR-002/003/004；AT-PRODUCT-001；AT-DATA-009；AT-UX-010 | migration `0015`；JourneyDefinition/Version/StageVersion；发布不可变触发器与复合范围约束；整版邀请/Enrollment/Assignment；按 position 的 Current Action 与最小进度；多阶段身份确认、等待不跳关、通过解锁下一阶段回归；见 35 | 本地 `0014↔0015`、空库迁移、Alpha 事实回填、269 项 API 与 Web 完整回归通过；干净镜像 Fast Gate 等待远端 CI；无部署、无真实数据改写；正式 Journey 邀请仍 fail closed | `READY_FOR_REMOTE_GATE / FORMAL_CONTENT_NOT_ACTIVATED / LIVE_ALPHA_UNCHANGED` |
