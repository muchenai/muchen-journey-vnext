...
| WP-12 候选硬化与灾备 | DEC-008/013/014/019；REQ-NFR-005/008/010；AT-DATA-008；AT-ISO-007；AT-ARCH-001/007 | 非本地默认身份/导入密钥无条件 fail closed；最小权限与 source map/依赖/secret audit；threat model；DEC-008 policy、migration `0014`、data-rights 台账和 PII-free plan；loopback-only read benchmark；加密本地备份、空白恢复、事实指纹和 `0013↔0014` 零新增事实回滚拒绝；`tests/test_wp12_*`；见 30 | 第一批硬化、threat model、数据生命周期 PLAN、本地性能和本地隔离恢复已验证；本地读取 p95 `0.0023–0.0145s`，恢复 `6.110s` 且不回滚已接受事实。APPLY/真实删除、staging benchmark、真实 RPO/RTO、真实故障卡、异机恢复仍未执行；独立故障域按 DEC-019 延期 | `IN_PROGRESS / LOCAL_ENGINEERING_BASELINE_PASS / DATA_LIFECYCLE_PLAN_ONLY / OFF_HOST_RESTORE_NOT_RUN / PRODUCTION_NO_GO`；不得记 `RC_TECHNICALLY_READY` |
