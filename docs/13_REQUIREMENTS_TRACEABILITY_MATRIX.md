...
| WP-12 候选硬化与灾备 | DEC-008/013/014/019；REQ-NFR-005/008/010；AT-DATA-008；AT-ISO-007；AT-ARCH-001/007 | 非本地默认身份/导入密钥无条件 fail closed；API/Web/Worker 最小权限；source map/依赖/secret/Python audit；仓库 threat model；DEC-008 固定 retention policy、migration `0014_wp12_data_lifecycle`、30 日 data-rights request 台账和 PII-free `PLAN_ONLY`；`tests/test_config.py`、`tests/test_wp12_candidate_hardening.py`、`tests/test_wp12_data_lifecycle.py`；见 30、`muchen-journey-vnext-threat-model.md` 和 `security_best_practices_report.md` | 第一批硬化与 threat model 已合入主线；数据生命周期计划/台账已实现但 APPLY、真实删除与 staging 运行仍未授权。独立灾备故障域按 DEC-019 延期。基础恢复、staging benchmark、RPO/RTO、N↔N+1 回滚及故障卡仍未关闭；异机恢复继续 `NOT_RUN` | `IN_PROGRESS / DATA_LIFECYCLE_PLAN_ONLY / DR_FAULT_DOMAIN_DEFERRED_FOR_ALPHA / PRODUCTION_NO_GO`；不得记 `RC_TECHNICALLY_READY` |
