...
| WP-08 物理独立 Staging | ISO-MUST-003/004/007/008/009/012；DEC-003/013/014；AT-ISO-002/003/005；AT-ARCH-002/003/005 | `make wp08-staging-readiness`、`make wp08-staging-apply-check`、`make wp08-workflow-check`、Terraform validate、saved-plan destroy/replacement guard、独立 Web `/health/ready` runtime check、匿名 `/ops = 401`、首次失败容器清理及候选源码 Web 合同门禁；DoR 与路径证据见 25/26 | provision 已冻结；run 30117658292 已完成 RDS TLS、migration 0001–0010、runtime grant、seed 和 API health，但 Web 因误用需身份 `/ops` 作为 healthcheck 而 unhealthy，edge/TLS 未启动，SSH 已关闭。修复把 readiness 与身份/API/DB 解耦，并拒绝新脚本与旧候选镜像混用 | `ALPHA_PILOT_WEB_READINESS_FIX_READY / NO_DEPLOY_CANDIDATE`；历史候选 670661… 禁止再次 deploy，合入后须生成并绑定新候选；真实身份、真人 Alpha UAT 与整体发布仍 `NO_GO` |
