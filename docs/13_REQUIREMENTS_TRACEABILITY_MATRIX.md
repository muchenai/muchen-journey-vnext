...
| WP-12 候选硬化与灾备 | DEC-008/013/014/019；REQ-NFR-005/008/010；AT-DATA-008；AT-ISO-007；AT-ARCH-001/007 | 非本地 fail closed；最小权限与 source map/依赖/secret audit；threat model；DEC-008 policy、migration `0014`、PII-free plan；Operator 数据权利登记/查询/legal hold/受限拒绝与脱敏审计；loopback-only benchmark；WP-12B 20 组织/500 Learner/50 并发真实 HTTP 负载（应用性能在北京现有 ECS API 容器内测量，公开 HTTPS 仅核对可达性/release）与数据库隔离审计；加密本地备份、空白恢复、事实指纹和 `0013↔0014` 零新增事实回滚拒绝；`tests/test_wp12_*`；见 30/32 | 硬化、threat model、生命周期 PLAN、Operator 台账、本地性能和本地隔离恢复已验证。候选 `674e51d8ed67f9c29c3d04693376c9ba6f1114e5` 经 Mainline run `30489417625` 和唯一 deploy run `30506961105` 成功后，WP-12B run `30508873351` 完成 20 组织/500 Learner、50 峰值并发和 10,561 请求。HTTP 5xx/409、跨组织泄漏、意外响应、数据库 cross-org mismatch/duplicate/incomplete 均为 0；560 个会话/用户退役、证据上传和 SSH 关闭均 PASS。性能门禁仍 FAIL：submission create p95=`1.119s`、review start=`1.309s`、review finalize=`1.423s`。隔离诊断已复现默认 15 连接池的 `0.750s` checkout wait p95；API `20+5`/Worker `2+1` 将其降至 `0.261s`，50 并发本地真实闭环的 Reviewer start/finalize p95 分别由 `0.464/0.765s` 降至 `0.284/0.362s`，并删除 submission 两个远端 flush 往返。新候选 `02863d0b670ee9b00b9def3e75bc6699827f555a` 已由 Mainline run `30511897160` 完成 registry 验证，并由唯一 deploy run `30519669770` 成功部署到冻结 staging；精确 digest、migration、全服务健康、外部 TLS/release surface 和 SSH 关闭均 PASS，WP-12B 尚未运行。物理 APPLY/真实删除、真实 RPO/RTO、真实故障卡、异机恢复仍未执行；独立故障域按 DEC-019 延期 | `IN_PROGRESS / WP12B_STAGING_LOAD_FAIL_NO_RETRY / LOCAL_SHARED_WRITE_DIAGNOSIS_COMPLETE / BOUNDED_POOL_CANDIDATE_DEPLOYED_WP12B_NOT_RUN / DATA_RIGHTS_LEDGER_READY_APPLY_DISABLED / OFF_HOST_RESTORE_NOT_RUN / PRODUCTION_NO_GO`；不得记 `RC_TECHNICALLY_READY` |
| WP-13 内容校准与真人 UAT | DEC-007/010/016；AT-UAT-001..008；AT-CONTENT-001..008；AT-UX-001/002/004/005/007 | 固定角色人数、8 场景、三类 Reviewer 校准、六项可访问性、5 秒理解阈值、Sev-1/2 和五角色签署的 fail-closed validator；见 31 | 仅合同与负测已完成；名册、冻结部署候选、真人执行与签署均未提供 | `PREPARED / HUMAN_UAT_NOT_RUN / NO_GO` |
...
| WP-15 生产切换与观察 | DEC-003/010/013/014；ISO-MUST-007/008/009/010/012 | 同一候选、18 项生产检查、两名不同角色/不同引用的批准、WP-13/14 前置；验证器无部署能力；见 31 | production 资源、真实通知/观测、异机恢复、批准、窗口、部署与观察均未授权/未执行 | `PRODUCTION_NO_GO / MUTATION_FALSE` |
