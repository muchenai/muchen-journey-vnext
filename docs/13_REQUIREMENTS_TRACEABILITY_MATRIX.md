...
| WP-09 真实身份与权限 | REQ-BR-002；DEC-006/014；AT-SEC-001/003/004/012；AT-ISO-003 | Feishu OAuth code exchange、HMAC subject、一次性 state + HttpOnly browser binding、return allowlist、session rotation/revoke、角色/组织/停用负测、代理回调和 callback 日志抑制；Operator 最小身份访问清单只显示同组织安全元数据和服务端 `allowed_commands`，支持一次性绑定与撤销；`tests/test_wp09_feishu_identity.py`、`tests/test_wp09_feishu_client.py`、OpenAPI/migration/Web build与 standalone 失效会话响应测试；见 27 | 独立飞书应用、callback、staging secrets、首个 Operator 与 Reviewer 已绑定；真人确认 Reviewer 授权页可达、`/ops` 被拒绝、重新登录轮换旧会话、撤销后旧会话不能继续；Operator 安全审计仅显示 allowlist 元数据和裁剪键。明确重新登录体验候选 `2ab2658…` 已完成完整 CI、三镜像和远端摘要验证，尚未部署 | `REAL_ACCESS_MATRIX_PASS / SESSION_EXPIRED_UX_CANDIDATE_BOUND`；WP-09 尚未关闭，整体发布仍 `NO_GO` |
