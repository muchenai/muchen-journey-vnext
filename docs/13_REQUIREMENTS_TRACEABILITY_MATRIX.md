...
| WP-09 真实身份与权限 | REQ-BR-002；DEC-006/014；AT-SEC-001/003/004/012；AT-ISO-003 | Feishu OAuth code exchange、HMAC subject、一次性 state + HttpOnly browser binding、return allowlist、session rotation/revoke、角色/组织/停用负测、代理回调和 callback 日志抑制；`tests/test_wp09_feishu_identity.py`、`tests/test_wp09_feishu_client.py`、OpenAPI/migration/Web build；见 27 | 独立飞书应用、callback、staging secrets、首个 Operator 绑定和真实 Reviewer/Operator 浏览器矩阵仍 `NOT_RUN`，执行前需对应 Owner 精确授权 | `LOCAL_IDENTITY_IMPLEMENTATION_VERIFIED / REAL_IDENTITY_NOT_RUN`；整体发布仍 `NO_GO` |
