...
| WP-12 候选硬化与灾备 | DEC-008/013/014/019；REQ-NFR-005/008/010；AT-DATA-008；AT-ISO-007；AT-ARCH-001/007 | 非本地默认身份/导入密钥无条件 fail closed；API/Web/Worker `no-new-privileges`、drop all capabilities、PID 上限；生产浏览器 source map 显式关闭并扫描构建产物；依赖/secret/Python audit；`tests/test_config.py`、`tests/test_wp12_candidate_hardening.py`、`make wp12-hardening-check`；见 30 和 `security_best_practices_report.md` | 第一批代码级硬化已合入主线；独立灾备故障域按 DEC-019 延期至真实 Alpha 连续稳定运行 30 日后的成熟检查点。基础备份/恢复、threat model、staging benchmark、保留删除、RPO/RTO、N↔N+1 回滚及故障卡仍未关闭；异机恢复继续 `NOT_RUN` | `IN_PROGRESS / DR_FAULT_DOMAIN_DEFERRED_FOR_ALPHA / PRODUCTION_NO_GO`；不得记 `RC_TECHNICALLY_READY` |
