...
| WP-12 候选硬化与灾备 | DEC-008/013/014；REQ-NFR-005/008/010；AT-DATA-008；AT-ISO-007；AT-ARCH-001/007 | 非本地默认身份/导入密钥无条件 fail closed；API/Web/Worker `no-new-privileges`、drop all capabilities、PID 上限；生产浏览器 source map 显式关闭并扫描构建产物；依赖/secret/Python audit；`tests/test_config.py`、`tests/test_wp12_candidate_hardening.py`、`make wp12-hardening-check`；见 30 和 `security_best_practices_report.md` | 第一批代码级硬化仅为本地候选证据；威胁模型上下文、staging benchmark、保留删除、异机加密副本与空白恢复、RPO/RTO、N↔N+1 回滚及故障卡仍未关闭；未部署 | `IN_PROGRESS / PRODUCTION_NO_GO`；不得记 `RC_TECHNICALLY_READY` |
