...
| WP-12 候选硬化与灾备 | DEC-008/013/014/019；REQ-NFR-005/008/010；AT-DATA-008；AT-ISO-007；AT-ARCH-001/007 | 非本地 fail closed；最小权限与 source map/依赖/secret audit；threat model；DEC-008 policy、migration `0014`、PII-free plan；Operator 数据权利登记/查询/legal hold/受限拒绝与脱敏审计；loopback-only benchmark；加密本地备份、空白恢复、事实指纹和 `0013↔0014` 零新增事实回滚拒绝；`tests/test_wp12_*`；见 30 | 硬化、threat model、生命周期 PLAN、Operator 台账、本地性能和本地隔离恢复已验证；本地读取 p95 `0.0023–0.0145s`，恢复 `6.110s` 且不回滚已接受事实。物理 APPLY/真实删除、staging benchmark、真实 RPO/RTO、真实故障卡、异机恢复仍未执行；独立故障域按 DEC-019 延期 | `IN_PROGRESS / LOCAL_ENGINEERING_BASELINE_PASS / DATA_RIGHTS_LEDGER_READY_APPLY_DISABLED / OFF_HOST_RESTORE_NOT_RUN / PRODUCTION_NO_GO`；不得记 `RC_TECHNICALLY_READY` |
