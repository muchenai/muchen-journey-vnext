...
状态：`WP19_22_MINIMAL_VERTICAL_SLICE_COMPLETE / MACHINE_READBACK_VERIFIED / PRODUCTION_NO_GO`。本文仍是 Greenfield vNext 唯一 staging 资源与部署入口；不复用旧 P1 脚本，不授权 production。Provision 已收敛并冻结。API/Worker 候选 `ef0a512…` 已由 run `30817611873` 部署，Edge 修复 run `30826160950` 将 staging upstream 收窄到唯一实例别名。2026-08-04，Web 候选 `12bc627…` 由唯一 run `30875911123` 完成 staging Web-only 部署；公开 readiness、根页面和匿名受保护路由均通过，数据库与业务事实未改变，SSH 已关闭。Operator 发布 Journey V1 后只生成一条绑定固定旅程的受控邀请；刷新 `/ops` 后 Journey V1 和状态“待使用”的邀请均由服务端读回，一次性链接正文未重复展示。内容真人门禁、WP-23 和 production 继续 `NO_GO`。
