...
- Content Editor OAuth 回调修复候选：`c0765eb625fc3c99205dc3d05abf9fad0475d81d`；PR #154 把 `/content` 加入 Web callback 的精确同源安全入口，并增加 cookie 响应转发合同。Mainline Candidate Gate `31171640166` 已完成完整 CI、SBOM、候选 manifest、三镜像 GHCR push 与远端 digest 复验，migration head 保持 `0019_wp30_invitation_control`；后续唯一 deploy run `31176970061` 成功且 SSH 已关闭，该候选已消费；
...
- Alpha 应用发布冻结现有基础设施：`phase=deploy` 只做 backend init、`terraform output -raw`、临时 SSH、发布包、前向 migration、runtime grant、Compose 与 TLS smoke，不运行 fixture seed、provider refresh、DNS import、plan 或 apply。部署不得创建或修复 Organization、User、RoleAssignment、Journey、Enrollment、Assignment、TaskVersion 或其他业务事实；基础设施变化只能显式使用 `phase=provision` 并继续通过 saved-plan 破坏性门禁。
...
当前 workflow/config 保留外部表面门禁候选 `ff53052…`、Mainline run `31340959377`、API `sha256:2a053bad…a6a6c`、Web `sha256:a3335542…e2aee`、Worker `sha256:2ef3cd1b…9f38`、artifact name 和已消费确认词作为不可变证据。唯一 deploy run `31342063864` 已成功将 staging 更新到 `ff53052…`，未创建邀请、未发送消息，也未修改已发布的 Journey V3 或既有 Enrollment。workflow 从 Git 历史核验候选源码本身包含 readiness、Compose 探针、`/ops`/`/review` 匿名 401、`/content` 匿名 303 同源登录恢复、Journey V3 邀请标签去重、逐项 `WP08_SURFACE_CHECK` 可观测输出、12 轮有界重试、请求 CSP nonce 传播、动态渲染、root-relative OAuth redirect、OAuth callback 安全入口与 cookie 响应转发、真实 standalone 失效会话响应测试、WP-11 通知/可观测接线合同、WP-12B 合成多租户工具，以及 API `20+5`/Worker `2+1` 连接池环境：
...
3. 所有历史确认词均不得复用；`DEPLOY_FF53052_TO_VOLCENGINE_STAGING` 已由 run `31342063864` 成功消费。工作流内部的有界重试只重读外部 HTTP 合同，不会再次部署；不得以复验或新主线 SHA 为由重新 dispatch 该候选。
