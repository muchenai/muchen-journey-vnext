...
状态：`WP19_22_INTERNAL_RUNTIME_READY / PUBLIC_STAGING_EDGE_ROUTE_REPAIR_PENDING / PRODUCTION_NO_GO`。本文仍是 Greenfield vNext 唯一 staging 资源与部署入口；不复用旧 P1 脚本，不授权 production。Provision 已收敛并冻结。候选 `ef0a512…` 已由 run `30817611873` 完成 Web/API/Worker/Edge 内部部署；PR #140 与只读 run `30823210293` 随后证明 staging 与 production Web 共同加入 `journey-next-staging_default` 时都会自动注册 Compose 服务名 `web`，而现行 Caddy staging upstream 仍为 `web:3000`，导致公开 staging 在新旧 Web 间非确定性路由。修复只允许把 staging upstream 收窄为 inventory 已验证的唯一别名 `journey-next-staging-web-1:3000`，以固定 Edge digest 只重建 Edge，并连续核验 staging 候选、production 既有 release 与匿名拒绝合同；失败必须恢复原 Caddyfile 和 Edge。Terraform、DNS、云资源、应用容器、数据库、消息、业务接收人和 WP-12B 均不在本次范围；production 继续 `NO_GO`。
