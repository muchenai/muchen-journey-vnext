...
状态：`WP12B_CANDIDATE_BINDING_READY / EXTERNAL_WRITES_NOT_AUTHORIZED`。本文仍是 Greenfield vNext 唯一 staging 资源与部署入口；不复用旧 P1 脚本，不授权 production。Provision 已收敛并冻结。staging 当前运行已消费候选 `172c9f6…`，唯一部署 run `30351059075` 已成功；该候选不得再次部署。WP-12B 工具已进入新候选 `9e1cdb2…`，但未授权部署或负载。
...
- 当前运行中已消费候选：`172c9f62ffdcd4fce31fb4900fdca46b3405ab89`；Mainline Candidate Gate `30302594972` 与唯一 deploy run `30351059075` 均成功，该候选不得再次部署；
- WP-12B 待授权候选：`9e1cdb280e47ecb5b2571a4f4bedb05a7c9f22f6`；Mainline Candidate Gate `30416410890` 完成 `ci-main`、SBOM、三镜像 GHCR push 与 registry digest 验证，manifest 明确 `deployment=NOT_RUN`；
...
3. 历史确认词 `DEPLOY_2AB2658_TO_VOLCENGINE_STAGING` 与 `DEPLOY_172C9F6_TO_VOLCENGINE_STAGING` 均已消费，不得复用。WP-12B 候选只接受 `DEPLOY_9E1CDB2_TO_VOLCENGINE_STAGING`；该文本不构成授权。获得当轮授权后，`phase=deploy` 仍只从冻结 state 读取既有 ECS/RDS 定位值，不执行 DNS、plan、apply 或 CloudControl；随后添加 runner 单一 `/32`，执行迁移、运行时授权、合成 seed、应用部署和 TLS 验证，并在 `always()` 步骤撤销该精确规则。
