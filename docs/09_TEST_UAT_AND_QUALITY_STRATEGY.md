...
WP-24 正式内容另加：`AT-WP24-001..008`。真人不得只验证“页面能打开”：必须从 Day 0 完成四个学习宝藏，独立作答三项题面，至少验证一次 Reviewer 修订，核对数值 Rubric，并由 Operator 在看见建议分档后作出一次人工准入决定。系统自动化只能证明状态、分数和权限合同，不能替代内容理解度、Reviewer 校准或人工准入判断。

正式探索营的角色与入口按下列用例验收，禁止把自证站完成误记为待人工评审：
