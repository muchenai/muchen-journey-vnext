...
原 WP 列表继续保留历史追溯，但未完成施工顺序由 43 号文档替代。P0-1 已由机器证据和同一飞书身份 `/content`、`/review` 真人复验关闭；当前唯一 WIP 是 P0-2 Learner 一站式任务页，通过后才进入 P0-3 完整真人闭环和 P0-4 受控观察。

新的完成单位是“一个目标用户在真实浏览器完成一个可观察结果”。模块实现、测试通过、PR 合并和部署成功分别记录，不得合并成一个“完成”。同类问题第二次出现即停止局部补丁并做模型级根因分析；不再等待第三次。
...
- 同类问题第二次出现即停止局部补丁，回到产品/领域/身份模型；第三次之前必须完成根因、简化方案与黄金路径回归；
