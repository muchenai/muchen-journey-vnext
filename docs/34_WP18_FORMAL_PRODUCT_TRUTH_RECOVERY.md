...
| 7 | `ASM-003-DATA-CONSTRUCTION` | 能力评测 | 我能否按固定结构、约束和覆盖要求构建一组可验收数据？ | 不可变实操作答 | Reviewer |
| 8 | `JOURNEY-OUTCOME` | 结果 | 我证明了什么，还需要发展什么，下一步去哪里？ | 三项评测的人工结论与完整旅程事实 | System + Reviewer |

四个宝藏不是装饰性关卡，而是让 Learner 获得完成能力评测所需的认知基础；三个能力评测不是知识问答，而是产生可由 Reviewer 复核的行为证据。DEC-025 进一步确认：边界识别与有效提报贯穿三项评测，不再单独占用第三项题面。
...
- 首屏主命题继续使用“这里，没有标准答案。”；“It's a long game.” 只作为低权重品牌暗线，不改变正式产品结构或阶段语义；
