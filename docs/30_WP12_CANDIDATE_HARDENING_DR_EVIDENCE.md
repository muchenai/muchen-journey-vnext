...
候选 `02863d0…` 已由唯一 run `30519669770` 部署，且 WP-12B run `30525165474` 完成正式规模：隔离、正确性、数据库审计和身份退役 PASS，原 1 秒性能合同因 submission/finalize p95=`1.012/1.097s` 保持 FAIL。DEC-020 不关闭 WP-12B，只按 p95≤1.2 秒为同一候选建立 WP-13 Alpha 条件入口；下一单一 WIP 转为真人 UAT。受控物理删除执行器继续作为 WP-12 未关闭高风险切片，独立故障域选型等到 DEC-019 的 30 日成熟检查点再重开。任何新的真实数据删除、备份目的地、KMS/存储资源或恢复演练均需另行获得精确授权。
