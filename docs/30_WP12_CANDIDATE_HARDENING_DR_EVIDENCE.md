...
下一单一 WIP 仍是关闭 WP-12B。API 20+5、Worker 2+1 的候选 `02863d0…` 已由唯一 run `30519669770` 成功部署到冻结 staging，外部 release surface 与 SSH 关闭均 PASS；该事实不能替代 staging 性能门禁。下一次外部写入只能是 Owner 对该候选和届时精确主线另行授权的一次 WP-12B。WP-12B 关闭前不启动 WP-13 真人 UAT。受控物理删除执行器继续作为 WP-12 未关闭高风险切片，独立故障域选型等到 DEC-019 的 30 日成熟检查点再重开。任何新的真实数据删除、备份目的地、KMS/存储资源或恢复演练均需另行获得精确授权。
