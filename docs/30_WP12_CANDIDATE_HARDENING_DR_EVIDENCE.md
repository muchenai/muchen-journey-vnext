...
下一单一 WIP 仍是关闭 WP-12B。隔离的 50 并发诊断已确认默认 15 连接池会产生约 0.75 秒 checkout wait p95；API 20+5、Worker 2+1 的有界配置把该等待降至约 0.26 秒，并在真实本地闭环中显著改善两条 Reviewer 写路径。当前只允许完成全量 CI、PR 合入和新候选生成；本地证据不能替代 staging 性能门禁。WP-12B 关闭前不启动 WP-13 真人 UAT。受控物理删除执行器继续作为 WP-12 未关闭高风险切片，独立故障域选型等到 DEC-019 的 30 日成熟检查点再重开。任何新的 staging 负载、真实数据删除、备份目的地、KMS/存储资源、staging 写入或恢复演练均需另行获得精确授权。
