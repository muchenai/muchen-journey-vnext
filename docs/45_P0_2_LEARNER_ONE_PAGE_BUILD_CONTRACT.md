...
首次 Web-only 尝试 `32455769220` 在不可变合同核验阶段 fail-closed，尚未打开 SSH 或修改运行态。根因为候选 registry/local 六项摘要未在 `wp08_prepare_deploy.py` 与 `deploy.sh` 中同步完整；修复要求机器合同同时核对 candidate manifest、准备脚本和部署脚本，而非只核对 Web 摘要。
