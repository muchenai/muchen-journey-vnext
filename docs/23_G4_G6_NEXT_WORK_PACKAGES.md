...
截至 2026-07-25，独立 staging 基础资源、RDS CA 和 remote state 已收敛，RDS 网络/CA、migration、runtime grant、seed、API 与 Web health 均已有物理证据。候选 `d407b5f…` 的唯一 deploy run `30138363837` 在 Worker 启动阶段失败：Worker 的数据库模块错误加载完整 API Settings，而最小权限 `worker.env` 不持有 Session/Invite/Import secret；首次失败容器清理和 SSH 撤销均通过，外部 TLS 未运行且没有重试。PR #41 已合入 database-only 配置边界；主线 Candidate Gate `30139385352` 为新候选 `dad44cc679184a1978b0f69e3632cb95de7f1b8e` 完成完整 CI、SBOM、GHCR push 与三摘要验证，机器合同已原子绑定且没有部署。当前状态为 `ALPHA_PILOT_CANDIDATE_BOUND / DEPLOY_NOT_AUTHORIZED`；必须另行取得指名新候选和绑定主线的精确 deploy 授权。真实身份、真人 UAT 与 WP-09 不得提前启动或转绿。
