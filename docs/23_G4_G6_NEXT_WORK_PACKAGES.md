...
变更说明：WP-07 已关闭；WP-08～WP-15 的编号、范围和顺序已批准，且用户已授权 WP-08～WP-13 按“独立任务、单一 WIP、主任务复验”自主推进。WP-08 的 Alpha staging 运行面已验证，物理 ACL 仍有一项供应商返回字段证据债；按 2026-07-25 止损决策，该证据债不再阻塞 WP-09 的代码实现和受控 Alpha 身份验证，但仍阻塞 RC/production。当前唯一活跃工作包仍为 WP-09：真人 Reviewer/Operator 的授权访问、越权拒绝、旧会话轮换、身份撤销和日志脱敏矩阵已通过；撤销会话被通用错误页掩盖的 UX 修复候选 `2ab2658…` 已完成完整 CI、三镜像与远端摘要验证，待冻结 staging 部署复验后关闭。WP-10～WP-15 尚未激活。外部权限、真人、时间窗口和生产写入仍须遵守各工作包的精确授权边界，计划批准不得替代这些授权。
