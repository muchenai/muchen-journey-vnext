...
变更说明：WP-07 已关闭；WP-08～WP-15 的编号、范围和顺序已批准，且用户已授权 WP-08～WP-13 按“独立任务、单一 WIP、主任务复验”自主推进。WP-08 的 Alpha staging 运行面已验证，物理 ACL 仍有一项供应商返回字段证据债；按 2026-07-25 止损决策，该证据债不再阻塞 WP-09 的代码实现和受控 Alpha 身份验证，但仍阻塞 RC/production。当前唯一活跃工作包为 WP-09：同源 OAuth 回跳修复已部署并由真实 Operator 验证自动进入 `/ops`，Reviewer/Operator 对象、组织、旧会话与撤销矩阵仍待执行；WP-10～WP-15 尚未激活。外部权限、真人、时间窗口和生产写入仍须遵守各工作包的精确授权边界，计划批准不得替代这些授权。
...
1. 当前只激活 WP-09；`ALPHA_STAGING_RUNTIME_VERIFIED` 足以进入真实身份与小规模 Alpha 学习，WP-08 的 `PHYSICAL_ACL_EVIDENCE_OPEN` 必须在 WP-12 RC 冻结或任何 production 行为前关闭；
