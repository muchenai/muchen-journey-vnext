...
`DEC-024/025` 锁定以下稳定内容身份；本节批准结构、目的与证据类型，不自动批准旧材料原文上线：
...
| `ASM-003-DATA-CONSTRUCTION` | 通用数据构建 | 证明能按 schema、约束和覆盖要求构建可验收数据并自检 | Reviewer Rubric + 可修订结论 |
...
TSK-001 V1 继续使用少量维度 + `PASS/REVISION_REQUIRED`，不计算总分。正式探索营 V2 按 DEC-025 使用固定 100 分证据与 A/B/C/D 建议分档；分数不是最终结论，准入只能由授权人工在看见证据后决定，覆盖建议必须记录理由。
